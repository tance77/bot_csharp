    ...
    internal class TwitchApi : WebFunctions
    {
        ...
        public bool CheckAccountCreation (string user)
        {
            try {
                ...
                if (DateTime.Compare (DateTime.Parse (creation_date).Date, DateTime.UtcNow.Date.AddDays (-182)) == 0) {
                    return false;
                }
        ...
        public bool CheckAccountCreationBot (string user)
        {
            try {
                string url = "https://api.twitch.tv/kraken/users/" + user;
                string jsonString = RequestJsonTwitch (url);
                ...
                if (DateTime.Compare (DateTime.Parse (creation_date).Date, DateTime.UtcNow.Date.AddDays (-128)) == 0) {
                    return true;
                }
                if (DateTime.Compare (DateTime.Parse (creation_date).Date, DateTime.UtcNow.Date.AddDays (-129)) == 0) {
                    return true;
                }
                if (DateTime.Compare (DateTime.Parse (creation_date).Date, DateTime.UtcNow.Date.AddDays (-144)) == 0) {
                    return true;
                }
                if (DateTime.Compare (DateTime.Parse (creation_date).Date, DateTime.UtcNow.Date.AddDays (-154)) == 0) {
                    return true;
                }
                if (DateTime.Compare (DateTime.Parse (creation_date).Date, DateTime.UtcNow.Date.AddDays (-132)) == 0) {
                    return true;
                }
                return false;
